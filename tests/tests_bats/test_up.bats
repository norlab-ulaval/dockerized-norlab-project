...
@test "dnp::up_command with --service option › expect service passed to up_and_attach" {
  # Test case: When up command is called with --service option, it should pass it to up_and_attach
  run bash -c "source ${MOCK_DNP_DIR}/src/lib/commands/up.bash && dnp::up_command --service project-custom"

  # Should succeed
  assert_success

  # Should output the expected message
  assert_output --partial "Mock dnp::up_and_attach called with args: --service project-custom"
}

@test "dnp::up_command with --service option and custom command › expect both passed to up_and_attach" {
  # Test case: When up command is called with --service option and custom command, it should pass both to up_and_attach
  run bash -c "source ${MOCK_DNP_DIR}/src/lib/commands/up.bash && dnp::up_command --service project-custom bash -c 'echo hello'"

  # Should succeed
  assert_success

  # Should output the expected message
  assert_output --partial "Mock dnp::up_and_attach called with args: --service project-custom bash -c echo hello"
}
