...
TESTED_FILE="super_project_dna_sanity_check.bash"
...
@test "dna::super_project_dna_sanity_check (sourced) › expect pass" {
  source "${BATS_DOCKER_WORKDIR}/src/lib/core/utils/import_dna_lib.bash" || exit 1
...
@test "dna::check_super_project_dir_structure › expect pass with valid structure" {
...
@test "dna::check_dockerized_project_configuration_dir_structure › expect pass with valid structure" {
  ...
  run dna::check_dockerized_project_configuration_dir_structure
  assert_success
}

@test "dna::check_dockerized_project_configuration_dir_structure › expect fail with missing configuration directory" {
...
@test "dna::check_project_configuration › expect pass with valid configuration" {
...
@test "dna::check_dockerignore › expect pass with valid dockerignore entries" {
  ...
  run dna::check_dockerignore
  assert_success
}

@test "dna::check_dockerignore › expect fail with missing required entries" {
