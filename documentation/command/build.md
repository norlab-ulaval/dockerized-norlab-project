...
| `--save DIRPATH` | Save built image to specified directory (develop, deploy or slurm services only)                                            |
| `--push` | Push image to Docker Hub (deploy or slurm services only, requires Docker Hub authentication). For slurm with `--apptainer`, selects the registry push pipeline (see `--apptainer`). |
