...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --help|-h)
      sed -n '/^# ={10}/,/^# ={10}/p' "${BASH_SOURCE[0]}" | grep '^#' | sed 's/^# \{0,1\}//'
      exit 0
      ;;
    --target-dir)
      if [[ -z "${2:-}" ]]; then
...
# Set APPTAINER_CACHEDIR and APPTAINER_TMPDIR to scratch space (SLURM_TMPDIR if available,
# otherwise system tmpdir) before loading the apptainer module to avoid Lustre home quota
# issues during SIF build. This mechanism works on all HPC servers.
...
# Use --mksquashfs-args for better compression if apptainer >= 1.4.0 supports it
...
if [[ "${_APPTAINER_MAJOR}" -gt 1 ]] || { [[ "${_APPTAINER_MAJOR}" -eq 1 ]] && [[ "${_APPTAINER_MINOR}" -ge 4 ]]; }; then
  echo "[info] Apptainer ${_APPTAINER_VERSION}: enabling zstd compression via --mksquashfs-args" 1>&2
  if ! apptainer build \
  ...
  echo "[info] Apptainer ${_APPTAINER_VERSION}: skipping --mksquashfs-args (requires >= 1.4.0)" 1>&2
  if ! apptainer build \
      "${SIF_TMP}" \
      "docker-archive:${TAR_FILE}"; then
    echo "[error] Apptainer build failed. The tar archive has been preserved: ${TAR_FILE}" 1>&2
...
#                 profile-specific logic. The SLURM_TMPDIR-aware tmpdir mechanism is used
#                 unconditionally on all HPC servers.
#
# Outputs:
...
#      then moves it to the final destination to avoid Lustre quota.
#
# Prerequisite: run dna_hpc_server_config.bash once to set up the directory structure
# and authenticate with the Docker registry.
#
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      echo "[error] Unknown argument: $1" 1>&2
      echo "Usage: $0 [--target-dir <PATH>] [--docker-login] [--help]" 1>&2
      exit 1
      ;;
  esac
done
...
# scratch space above (SLURM_TMPDIR-aware mktemp), so the cache never lands in the home
...
#
# Usage (from the super-project root directory on the HPC server):
#   $ bash artifact/apptainer/dna_hpc_server_config.bash [--target-dir <TARGET-DIRECTORY-PATH>]
#
# Options:
#   --target-dir <TARGET-DIRECTORY-PATH>
#       Optional. Path to the HPC super-project root directory.
#       When specified, the directory structure will be created under this path.
#       Defaults to two levels above this script's directory (i.e., the super-project root
#       inferred from the standard artifact/apptainer/ placement).
#   --help
#       Print this help message and exit.
#
# Requires:
