...
# =================================================================================================
# Generates a build_sif.sh helper script to be run on the HPC server.
#
...
#   output_dir    - Directory where the build_sif.sh script will be written
#
# Outputs:
#   Writes build_sif.sh to output_dir
#
# Returns:
...
function dna::generate_apptainer_build_sif_script() {
  ...
  local script_path="${output_dir}/build_sif.sh"
...
# Run this script on the HPC server AFTER transferring the tar archive.
#
# Usage:
#   \$ bash build_sif.sh
#
...
#   - ${tar_filename} in the same directory as this script
#
# =================================================================================================
set -e
...
echo "[info] Building Apptainer SIF from Docker tar archive..."
echo "[info]   Input:  \${TAR_FILE}"
echo "[info]   Output: \${SIF_FILE}"

apptainer build "\${SIF_FILE}" "docker-archive:\${TAR_FILE}"

echo "[done] SIF file created: \${SIF_FILE}"
EOF
...
#
# Positional arguments:
#   sjob_id     - Slurm job ID (used for naming the script and container)
#   profile     - HPC server profile name (e.g., 'valeria', 'compute_canada', 'mamba')
...
#
# Outputs:
#   Writes run_apptainer_<sjob_id>.sh to output_dir
...
function dna::generate_apptainer_run_script() {
  ...
  local script_name="run_apptainer_${sjob_id}.sh"
...
#!/bin/bash
# =================================================================================================
# Auto-generated by DNA: dna run slurm ${sjob_id} --generate-apptainer ${profile}
...
if [[ ! -f "\${SIF_PATH}" ]]; then
  ...
  echo "[hint] Build it with: bash build_sif.sh" 1>&2
  exit 1
fi
