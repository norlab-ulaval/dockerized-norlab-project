...
SCRIPT_EOF
  ...
  # Inject profile-specific Apptainer cache configuration
...
if ! apptainer build "${SIF_FILE}" "docker-archive:${TAR_FILE}"; then
...
# It pulls the Docker image from the registry and converts it to an Apptainer SIF file.
#
# Unlike dna_tar_to_apptainer_sif_converter.sh (which works with a local .tar archive), this
# script uses 'apptainer pull docker://' to fetch the image directly from a Docker registry.
#
...
#
# This script:
#   1. Optionally authenticates to the Docker registry interactively via 'apptainer pull --docker-login'.
#   2. Pulls the Docker image from docker.io and converts it to an Apptainer SIF file.
#   3. Sets up and validates the expected super-project directory structure on the HPC server.
#
# Usage (from the super-project root directory on the HPC server):
...
#       (docker.io) before pulling the image. Apptainer will prompt for credentials.
#
# Requires:
#   - apptainer installed on the HPC server
...
SCRIPT_EOF
  ...
  cat >> "${script_path}" << 'SCRIPT_EOF'
# ====Pull image from registry and convert to SIF==================================================
APPTAINER_PULL_CMD=(apptainer pull --disable-cache)
if [[ "${USE_DOCKER_LOGIN}" == true ]]; then
  echo "[info] Authenticating with docker.io interactively (--docker-login)..." 1>&2
  APPTAINER_PULL_CMD+=(--docker-login)
else
  echo "[info] No --docker-login flag provided — assuming public registry access." 1>&2
fi

echo "[info] Pulling image and building Apptainer SIF..." 1>&2
echo "[info]   Source: docker://${IMAGE_REF}" 1>&2
echo "[info]   Output: ${SIF_FILE}" 1>&2

if ! "${APPTAINER_PULL_CMD[@]}" "${SIF_FILE}" "docker://${IMAGE_REF}"; then
  echo "[error] Apptainer pull/build failed for image: ${IMAGE_REF}" 1>&2
