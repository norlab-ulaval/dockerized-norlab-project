...
function dnp::project_validate_slurm() {
  ...
  for each_compose in "${dryrun_compose_file_list[@]}"; do
    ...
    if [[ "${each_compose}" =~ .*".multiarch.yaml" ]]; then
      dnp::build_dn_project_multiarch_services "${add_fct_flag[@]}" --msg-line-level "${MSG_LINE_CHAR_BUILDER_LVL2}" "${build_flag[@]}"
    else
      dnp::build_dn_project_services "${add_fct_flag[@]}" --msg-line-level "${MSG_LINE_CHAR_BUILDER_LVL2}" "${build_flag[@]}"
    fi
