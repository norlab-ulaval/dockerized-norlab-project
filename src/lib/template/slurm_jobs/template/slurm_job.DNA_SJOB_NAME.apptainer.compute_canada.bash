...
#                    artifact/apptainer/compute_canada/, slurm_jobs/slurm_job.<DNA_SJOB_NAME>.apptainer.compute_canada.bash,
#                    .dockerized_norlab/,
#                    data/external_data/, data/repository_data/
#                    (data/shared_data/ is optional — replaced by a local data volume on the HPC server)
#     On Compute Canada:
#       4. Build SIF: bash artifact/apptainer/compute_canada/dna_tar_to_apptainer_sif_converter.sh
...
#                    (data/shared_data/ is optional — replaced by a local data volume on the HPC server)
#     On Compute Canada:
#       4. Build SIF: bash artifact/apptainer/compute_canada/dna_registry_to_apptainer_sif_converter.sh
...
# ...-slurm-l4t-r36.4.0-compute-canada.sif). The exact version is only known at build time, so resolve
# the newest matching versioned SIF at runtime. Export SIF_PATH to pin a specific version instead.
SIF_PATH="${SIF_PATH:-$(ls -t ${SCRATCH}/sif/PLACEHOLDER_DN_PROJECT_IMAGE_NAME-slurm-*-compute-canada.sif 2>/dev/null | head -n1)}"
...
if [[ ! -f "${SIF_PATH}" ]]; then
  ...
  echo "[hint] Build it with: bash artifact/apptainer/compute_canada/dna_tar_to_apptainer_sif_converter.sh" 1>&2
  exit 1
fi
