...
function dna::load_command() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --help|-h)
                dna::command_help_menu "${DOCUMENTATION_BUFFER_LOAD}"
                exit 0
                ;;
            *)
                ...
                if [[ -z "${save_dir_path}" ]]; then
                    ...
                    dna::illegal_command_msg "load" "${original_command}" "Unknown argument: $1\n"
                    return 1
                fi
                ;;
        esac
    ...
    if [[ -z "${save_dir_path}" ]]; then
        # No SAVE_DIR_PATH provided, assume we are already in a 'dna save' directory
        ...
        if [[ -f "meta.txt" ]]; then
            ...
            # Try to find super project root (assume its a dna saved deploy project)
            local original_cwd
            original_cwd="$(pwd)"

            # Source the load_super_project_config to get access to dna::cd_to_dna_super_project_root
            source "${DNA_LIB_PATH}/core/utils/load_super_project_config.bash" --no-execute || return 1

            if dna::cd_to_dna_super_project_root; then
                if [[ -f "meta.txt" ]]; then
                    save_dir_path="$(pwd)"
...
"
    ...
    if [[ "${service}" == "deploy" ]]; then
        dna::handle_deploy_post_load "${save_dir_path}" "${repo_name}" || {
            n2st::print_msg_error "Failed to handle deploy post-load actions"
            return 1
        }
    elif [[ "${service}" == "develop" ]]; then
        dna::handle_develop_post_load "${alias_prefix}" || {
...
#   $2: Repository name
# =================================================================================================
function dna::handle_deploy_post_load() {
...
You can then run 'dna up deploy' or 'dna run deploy' commands"
    return 0
}
...
# =================================================================================================
function dna::handle_develop_post_load() {
    local alias_prefix="$1"

    if [[ -n "${alias_prefix}" ]]; then
        local alias_command="dna-${alias_prefix}-cd"
    ...
    Please manually navigate to your project directory and run ${MSG_DIMMED_FORMAT}dna [up|run] develop${MSG_END_FORMAT}"
        fi
    else
