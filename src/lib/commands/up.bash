...
function dna::up_command() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --help|-h)
                dna::command_help_menu "${DOCUMENTATION_BUFFER_UP:?err}"
                exit 0
                ;;
            develop|deploy)
                service="$1"
                ...
                ;;
            --no-up)
                dna::illegal_command_msg "up" "--no-up" "Its a dna internal flag"
                exit 1
                ;;
            *)
    ...
    if [[ ${service_override} -ge 2 ]]; then
        ...
        dna::illegal_command_msg "up" "${original_command}" "Only one SERVICE can be specified.\n"
        return 1
    fi
    ...
    # ....Begin....................................................................................
    dna::up_and_attach "${remaining_args[@]}"
    fct_exit_code=$?
